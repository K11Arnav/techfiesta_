...
import { ShieldAlert, Play, Pause, Activity, FileText, Gauge, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export default function FraudPage() {
    ...
    const processNextTransaction = async () => {
        ...
        try {
            const res = await authFetch('http://localhost:8000/score/xgb', {
                method: 'POST',
            ...
            console.error('Error in XGB stream:', err);
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                    <div>
                        <h1 className="text-4xl font-bold bg-gradient-to-r from-zinc-50 to-zinc-400 bg-clip-text text-transparent mb-2">
                            Fraud Detection - XGBoost Engine
                        </h1>
                        <p className="text-zinc-400 max-w-2xl">
                            Analyzing transaction features for gradient-boosted decision transparency.
                        </p>
                    </div>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        {!score && !currentTxn ? (
                            ...
                            <div className="space-y-6">
                                ...
                                <div className={`p-6 rounded-3xl border ${score && score >= 0.8 ? 'bg-rose-500/10 border-rose-500/20' : score && score >= 0.6 ? 'bg-amber-500/10 border-amber-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                                    ...
                                    <div className="flex items-end justify-between mb-4">
                                        ...
                                        <div className={`text-xl font-bold uppercase tracking-wider ${score && score >= 0.8 ? 'text-rose-400' : score && score >= 0.6 ? 'text-amber-400' : 'text-emerald-400'}`}>
                                            {score && score >= 0.8 ? 'Blocked' : score && score >= 0.6 ? 'Flagged' : 'Safe'}
                                        </div>
                                    </div>
                                ...
                                {/* Engine Contribution - Specific to XGB */}
                                <div className="p-6 rounded-3xl bg-zinc-900/50 border border-zinc-800">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <span className="text-xs font-bold uppercase px-3 py-1.5 rounded-lg border bg-emerald-500/20 text-emerald-400 border-emerald-500/30">
                                            XGBoost 100%
                                        </span>
                                        ...
                                        <div className="space-y-2">
                                            {explainabilityV2.executive_summary.split('. ').filter(Boolean).map((sentence, i) => (
                                                <div key={i} className="flex items-start gap-2 text-xs">
                                                    <span className={`mt-0.5 font-bold ${sentence.toLowerCase().includes('below') || sentence.toLowerCase().includes('safe') || sentence.toLowerCase().includes('cleared') ? 'text-emerald-400' : 'text-rose-400'}`}>
                                                        {sentence.toLowerCase().includes('below') || sentence.toLowerCase().includes('safe') || sentence.toLowerCase().includes('cleared') ? '✔' : '⚠'}
                                                    </span>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8">
                            <div className="flex items-center gap-2 mb-8">
                                ...
                                <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Statistical Risk Drivers (SHAP)</h3>
                            </div>
                            ...
                            {explanation.length === 0 ? (
                                <div className="h-64 flex flex-col items-center justify-center text-zinc-600 border border-dashed border-zinc-800 rounded-2xl">
                                    ...
                                    <p className="text-sm tracking-widest">PENDING MODEL INFERENCE</p>
                                </div>
                            ) : (
                                <div className="space-y-6">
                                    {explanation.map((item, idx) => (
                                        <div key={idx}>
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                <span className={`text-[11px] font-mono font-bold ${item.impact > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                                            ...
                                            <div className="h-2 w-full bg-zinc-950 rounded-full border border-zinc-800 overflow-hidden">
                                                <motion.div
                                                    initial={{ width: 0 }}
                                                    animate={{ width: `${Math.min(Math.abs(item.impact) * 25, 100)}%` }}
                                                    className={`h-full rounded-full transition-all duration-300 ${item.impact > 0 ? 'bg-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.4)]' : 'bg-emerald-500'}`}
                                                />
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8">
                            <div className="flex items-center gap-2 mb-8">
                                ...
                                <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Transaction Forensic Table</h3>
                            </div>
                            ...
                            {!currentTxn ? (
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                                    {Object.entries(currentTxn).filter(([k]) => ['Time', 'Amount', 'V1', 'V4', 'V7', 'V10', 'V12', 'V17'].includes(k)).map(([key, val]: [string, any]) => (
